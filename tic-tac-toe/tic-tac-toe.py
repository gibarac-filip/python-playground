...
class QLearningAgent:
    ...
    def update_q_value(self, state, action, reward, next_state):
        ...
        possible_actions_next = env.get_possible_actions(next_state)
...
def main():
    ...
    # Training loop
    global episodes
    episodes = 362880  # Max potential actions allowed for  3x3 game, 362880
    evaluation_interval = 50000  # Set your desired evaluation interval
    ...
    for episode in tqdm(range(episodes), desc="Training", unit=" episodes"):
        ...
        while True:
            ...
            available_actions = env.get_possible_actions(state)
...
def evaluate_agent(agent, env, player_symbol='X'):
    ...
    for _ in tqdm(range(episodes), desc="Evaluating", unit=" episodes"):
        ...
        while True:
            available_actions = env.get_possible_actions(state)
            action = agent.choose_action(state, available_actions, np.zeros((3, 3)))  # Pass a dummy matrix for moves
            
            # Take the chosen action with the specified player symbol
            env.make_move(action[0], action[1], player_symbol, 0)
            next_state = tuple(map(tuple, env.board.copy()))
